...
export default function FunnelClient({ data, slug, refCode }) {
  ...
  const product = useMemo(() => localizeProduct(data.product, lang), [data.product, lang])
  ...
  const hasIntro = hasText(product.description) || hasText(product.priceNote)
  ...
  return <main className="lf-funnel">
    ...
    {hasIntro && <section id="intro" className="lf-section lf-intro">{hasText(product.description) && <><h2>{t(lang, 'intro')}</h2><div>{renderRich(product.description)}</div></>}{hasText(product.priceNote) && <b className="lf-price-note">{product.priceNote}</b>}</section>}
    ...
    {hasCta && <section className="lf-final-cta">
      <p>{t(lang, 'readyKicker')}</p>{hasText(ctaSection?.title) && <h2>{ctaSection.title}</h2>}
