...
export default function MemberDashboard() {
  ...
  return <main className="lf-app-shell">
    ...
    {active === 'menu' && <>
      <section className="lf-page-title"><h1>{mt('drawerTitle')}</h1><p>{mt('kycHint')}</p></section>
      <section className="lf-soft-card">
      ...
      <section className="lf-soft-card">
        <button className="lf-main-btn" onClick={logout}>{mt('logout')}</button>
      </section>
    </>}
    ...
    {drawerOpen && <div className="lf-drawer-backdrop" onClick={() => setDrawerOpen(false)}>
      <aside className="lf-drawer" onClick={e => e.stopPropagation()}>
        ...
        <section className="lf-drawer-profile">
          <b>{tierIcon[member.memberTier] || '✨'} {localizedTierText(member.memberTier, mt)}</b>
          <small>{mt('bonusCap')}: {bonusCapPercent}%</small>
          <div className="lf-drawer-credit-row">
            <span>{mt('paidShort')}: RM {money(member.paidCredit)}</span>
            <span>{mt('bonusShort')}: RM {money(member.bonusCredit)}</span>
          </div>
        </section>

        <details className="lf-drawer-details" open>
        ...
        <div className="lf-menu-list lf-clean-menu">
...
.lf-drawer-profile{margin:6px 0 14px;padding:15px;border:1px solid #e6ebf5;border-radius:22px;background:linear-gradient(135deg,#f8faff,#f4efff)}.lf-drawer-profile b{display:block;font-size:16px}.lf-drawer-profile small{display:block;color:#64748b;margin-top:5px;font-weight:900}.lf-drawer-credit-row{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}.lf-drawer-credit-row span{display:block;border:1px solid #e8edf7;background:#fff;border-radius:14px;padding:10px;font-size:12px;font-weight:900;color:#334155}.lf-clean-menu button{background:linear-gradient(135deg,#fff1f2,#fff);color:#9f1239;border-color:#fecdd3}
