...
const MEMBER_TEXT = {
  zh: {
    ...
    home:'Home', earn:'Earn', store:'Store', funnel:'Funnel', price:'Price', bonusMax:'Bonus max', needPaid:'Need Paid', currentPlan:'Current Plan', switchPlan:'Switch Plan', activateMonthly:'Activate Monthly', buyNow:'Buy Now', alreadyActivated:'已开通，不重复扣款',
  ...
  en: {
    ...
    home:'Home', earn:'Earn', store:'Store', funnel:'Funnel', price:'Price', bonusMax:'Bonus max', needPaid:'Need Paid', currentPlan:'Current Plan', switchPlan:'Switch Plan', activateMonthly:'Activate Monthly', buyNow:'Buy Now', alreadyActivated:'Already active, no duplicate charge',
  ...
  bm: {
    ...
    home:'Home', earn:'Earn', store:'Store', funnel:'Funnel', price:'Price', bonusMax:'Bonus max', needPaid:'Need Paid', currentPlan:'Current Plan', switchPlan:'Tukar Plan', activateMonthly:'Aktif Bulanan', buyNow:'Beli Sekarang', alreadyActivated:'Sudah aktif, tiada caj berulang',
...
export default function MemberDashboard() {
  ...
  return <main className="lf-app-shell">
    ...
    <header className="lf-mobile-header">
      <button className="lf-icon-btn" onClick={() => setActive('menu')} aria-label={mt('menu')}>☰</button>
      <div>
    ...
    {active === 'home' && <>
      ...
      <section className="lf-soft-card lf-bonus-card">
        <div className="lf-row between">
          ...
          <button className="lf-text-btn" onClick={() => setActive('menu')}>{mt('viewDetails')}</button>
        </div>
      ...
      <section className="lf-soft-card mission-preview">
        ...
        <button className="lf-light-btn" onClick={() => setActive('earn')}>{mt('submitStoryProof')}</button>
      </section>

      <QuickActions setActive={setActive} mt={mt} />
    ...
    {active === 'funnel' && <>
      <section className="lf-page-title"><h1>{mt('aiFunnel')}</h1><p>{mt('aiFunnelDesc')}</p></section>
      <section className="lf-soft-card">
    ...
    <nav className="lf-bottom-nav">
      <NavButton active={active === 'home'} icon="🏠" label={mt('home')} onClick={() => setActive('home')} />
      <NavButton active={active === 'earn'} icon="🎯" label={mt('earn')} onClick={() => setActive('earn')} />
      <NavButton active={active === 'store'} icon="🛒" label={mt('store')} onClick={() => setActive('store')} />
      <NavButton active={active === 'wallet'} icon="💳" label={mt('wallet')} onClick={() => setActive('wallet')} />
      <NavButton active={active === 'funnel'} icon="🚀" label={mt('funnel')} onClick={() => setActive('funnel')} />
      <NavButton active={active === 'menu'} icon="☰" label={mt('menu')} onClick={() => setActive('menu')} />
    </nav>
  </section>
  </main>
}
...
function DesktopSidebar({ active, setActive, member, mt, logout }) {
  const items = [
    ['home','⌂',mt('home')], ['store','◈',mt('productStore')], ['earn','◎',mt('earn')], ['wallet','▣',mt('wallet')], ['funnel','△',mt('funnel')], ['menu','☰',mt('menu')]
  ]
  return <aside className="lf-desktop-sidebar">
    ...
    <div className="lf-side-card">
      ...
      <button onClick={() => setActive('menu')}>View Benefits</button>
    </div>
...
function QuickActions({ setActive, mt }) {
  return <section className="lf-quick-row">
    <button onClick={() => setActive('earn')}><span>📤</span>{mt('quickUpload')}</button>
    <button onClick={() => setActive('earn')}><span>👥</span>{mt('inviteFriends')}</button>
    <button onClick={() => setActive('earn')}><span>🖼️</span>{mt('materialLibrary')}</button>
    <button onClick={() => setActive('store')}><span>🛒</span>{mt('buyService')}</button>
  </section>
}
